...
public class CommonLlmChatModel implements ChatModel {
    ...
    private ChatResponse internalCall(Prompt prompt, ChatResponse previousChatResponse) {
        ...
        List<Generation> generations = choices.stream().map(choice -> {
            ...
            if (StringUtils.isBlank(content)) {
                ...
                reasoning_content = Optional.ofNullable(choice.getDelta()).map(CommonLlmApi.Delta::getReasoning_content).orElse("");
            }
    ...
    //     List<CommonLlmApi.ChatCompletionMessage> messages = prompt.getInstructions()
    //             .stream()
    //             .map(message -> {
    //                 CommonLlmApi.ChatCompletionMessage apiMessage =
    //                         new CommonLlmApi.ChatCompletionMessage();
    ...
    //
    //     CommonLlmApi.ChatCompletionRequest request =
    //             new CommonLlmApi.ChatCompletionRequest();
    ...
    private CommonLlmApi.ChatCompletionRequest createRequest(Prompt prompt, boolean stream) {
        ...
        List<CommonLlmApi.ChatCompletionMessage> messages = prompt.getInstructions().stream().map(message -> {
            if (message.getMessageType() == MessageType.USER || message.getMessageType() == MessageType.SYSTEM) {
                return List.of(new CommonLlmApi.ChatCompletionMessage(
                        message.getText(),
                ...
                return List.of(new CommonLlmApi.ChatCompletionMessage(
                        ...
                        message.getMessageType().getValue(), toolCalls));
            } else if (message.getMessageType() == MessageType.TOOL) {
                ToolResponseMessage toolMessage = (ToolResponseMessage) message;

                toolMessage.getResponses()
                        .forEach(response -> Assert.isTrue(response.id() != null, "ToolResponseMessage must have an id"));
                List<CommonLlmApi.ChatCompletionMessage> list = toolMessage.getResponses()
                        .stream()
                        .map(tr -> {
                            return new CommonLlmApi.ChatCompletionMessage(
                                    tr.responseData(),
    ...
    private List<CommonLlmApi.FunctionTool> getFunctionTools(List<ToolDefinition> toolDefinitions) {
        return toolDefinitions.stream().map(toolDefinition -> {
            var function = new CommonLlmApi.FunctionTool.Function(toolDefinition.description(), toolDefinition.name(),
